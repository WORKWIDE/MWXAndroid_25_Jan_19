...
var app = {
    ...
    onDeviceReady: function() {
        ...
        //StatusBar.backgroundColorByHexString('#CACFD2');
                        ...
                        if(updateMapInterval!=null){
                            ...
                            updateMapInterval=null;
                                      ...
                                      if (pid == "category_view_page") {
                                          ...
                                          //window.close();
                                      }else if(pid=="Accepted_Start_work_view_tasklist_page" || pid=="offline_direct_work_start" || pid=="Landingpage" || pid=="offline_Landingpage"  ){
                                          return;
                                      }else if(pid=="attachmentPage"  ){
                                           changePage("#Accepted_Start_work_view_tasklist_page", "slide", false);
                                           return;
                                      }else if(pid=="attachmentPage_offline"  ){
...
function PushNotificationRegistration() {
    ...
    push.on('notification', function(data) {
        ...
        if (data.additionalData.foreground) {
            ...
            x.className = "show";
            setTimeout(function() {
                x.className = String(x.className).replace("show", "");
            }, 4000);
        } else {
...
$(document).on("pageshow", function(event, data) {
    ...
    switch (activePage.attr('id')) {
        ...
        case 'offline_Landingpage':
            ...
            $(".imageprofile_offline").css("background-image", "url(" + cordova.file.applicationStorageDirectory + "files/files/" + "profile.png)");
...
function current_position() {
    ...
    navigator.geolocation.getCurrentPosition(showPosition, errorlocation, options);
...
function loginIn() {
    ...
    if (($("#userName").val() == '') && ($("#password").val() == ''))
        ...
        showAlert("Please Enter Username", "WorkWide");
    else if ($("#password").val() == '')
        showAlert("Please Enter Password", "WorkWide");
    else {
...
function changePassword() {
    ...
    if (($("#forgotPasswordPgOldPassword").val() == '') && ($("#forgotPasswordPgNewPassword").val() == '') && ($("#forgotPasswordPgConfirmNewPassword").val() == ''))
        showAlert("Please Enter All Fields", "WorkWide");
    else if ($("#forgotPasswordPgOldPassword").val() == '')
        showAlert("Please Enter Old Password", "WorkWide");
    else if ($("#forgotPasswordPgNewPassword").val() == '')
        showAlert("Please Enter New Password", "WorkWide");
    else if ($("#forgotPasswordPgConfirmNewPassword").val() == '')
        showAlert("Please Enter Confirm New Password", "WorkWide");
    else if ((localStorage.getItem("password")) != ($("#forgotPasswordPgOldPassword").val()))
        showAlert("Old Password Doesn't Match", "WorkWide");
    ...
    QuinticaWebService("GET", "task_list", parameters, function(response) {
        ...
        if (response.code == "1") {
            ...
            for (var i = 0; i < Object.keys(response.data).length; i++) {
                ...
                if(!critical){
                     $("#task_view_accepted").append("<hr  data-icon='false' style='background-color: #ff0000; height: 2px; width: 50%; padding-bottom: 20x;'><h3 style='width: 100px; margin: auto;text-align: center; position: relative; padding-bottom: 15px;'>"+response.data[i].priority_type+"</h3>");
                     critical=true;
                ...
                if(!heigh){
                     $("#task_view_accepted").append("<hr data-icon='false' style='margin-top: 5%; background-color: #ff7e00; height: 2px; width: 50%;'><h3 style='width: 100px; margin: auto;text-align: center; position: relative; padding-bottom: 15px;'>"+response.data[i].priority_type+"</h3>");
                     heigh=true;
                ...
                if(!moderate){
                     $("#task_view_accepted").append("<hr data-icon='false' style='margin-top: 5%; background-color: #0c6410; height: 2px; width: 50%;'><h3 style='width: 100px; margin: auto;text-align: center; position: relative; padding-bottom: 15px;'>"+response.data[i].priority_type+"</h3>");
                     moderate=true;
                }
                }
                if(response.data[i].priority=='4'){
                if(!low){
                     $("#task_view_accepted").append("<hr  data-icon='false' style='margin-top: 5%; background-color: #0836e2; height: 2px; width: 50%;'><h3 style='width: 100px; margin: auto;text-align: center; position: relative; padding-bottom: 15px;'>"+response.data[i].priority_type+"</h3>");
                     low=true;
                }
                }
                ...
                if(!planing){
                      $("#task_view_accepted").append("<hr data-icon='false' style='margin-top: 5%; background-color: #8808e2; height: 2px; width: 50%;'><h3 style='width: 100px; margin: auto;text-align: center; position: relative; padding-bottom: 15px;'>"+response.data[i].priority_type+"</h3>");
                      planing=true;
                }
                }
                                var count=0;
...
function initMap(map_id, distance_id,taskIDs) {
   ...
   /* directionsDisplay.setOptions({
    ...
    if(distance_id=='distancess'){
        updateMapInterval = setInterval(function(){ calculateAndDisplayRoute(directionsService, directionsDisplay,mapss,distance_id,taskIDs); }, 60000);
    }
}
...
function calculateAndDisplayRouteForMultipleDestination(directionsService, directionsDisplay, map) {
    ...
    /*    var checkboxArray = document.getElementById('waypoints');
        for (var i = 0; i < checkboxArray.length; i++) {
          if (checkboxArray.options[i].selected) {*/
            waypts.push({
              location: new google.maps.LatLng(18.457532, 73.867746),
              stopover: true
            },
            {
              location: new google.maps.LatLng(18.170500, 73.662451),
              stopover: true
            },
            {
              location: new google.maps.LatLng(18.508920, 73.926026),
              stopover: true
            });
          /*}
        }*/
...
function Get_time_distance(taskIDs) {
    ...
    QuinticaWebService("GET", "distanceCalc", parameters, function(response) {
        ...
        if (response.code == "1" && response.data.status != "NOT_FOUND") {
            ...
            //document.getElementById("time").innerHTML = "Total-Time: " + response.data.duration.text;
            document.getElementById("distance").innerHTML = "This trip will take you " + response.data.duration.text + "(" + response.data.distance.text + ")";
                    ...
                    for (k = 0; k < Object.keys(response.data).length; k++) {
                        ...
                        var d = new Date(response.data[k].created_date);
                         ...
                         if(response.data[k].priority=='1'){
                                        if(!critical){
                                             $("#Accepted_Tasks_list").append("<hr  data-icon='false' style='background-color: #ff0000; height: 2px; width: 50%; padding-bottom: 20x;'><h3 style='width: 100px; margin: auto;text-align: center; position: relative; padding-bottom: 15px;'>"+response.data[k].priority_type+"</h3>");
                                             critical=true;
                                        }
                                        ...
                                        if(!heigh){
                                             $("#Accepted_Tasks_list").append("<hr data-icon='false' style=' margin-top: 5%; background-color: #ff7e00; height: 2px; width: 50%;'><h3 style='width: 100px; margin: auto;text-align: center; position: relative; padding-bottom: 15px;'>"+response.data[k].priority_type+"</h3>");
                                             heigh=true;
                                        }
                                        }
                                        ...
                                        if(!moderate){
                                             $("#Accepted_Tasks_list").append("<hr data-icon='false' style=' margin-top: 5%; background-color: #0c6410; height: 2px; width: 50%;'><h3 style='width: 100px; margin: auto;text-align: center; position: relative; padding-bottom: 15px;'>"+response.data[k].priority_type+"</h3>");
                                             moderate=true;
                                        }
                                        }
                                        if(response.data[k].priority=='4'){
                                        if(!low){
                                             $("#Accepted_Tasks_list").append("<hr  data-icon='false' style='margin-top: 5%; background-color: #0836e2; height: 2px; width: 50%;'><h3 style='width: 100px; margin: auto;text-align: center; position: relative; padding-bottom: 15px;'>"+response.data[k].priority_type+"</h3>");
                                             low=true;
                                        }
                                        }
                                        ...
                                        if(!planing){
                                              $("#Accepted_Tasks_list").append("<hr data-icon='false' style='margin-top: 5%; background-color: #8808e2; height: 2px; width: 50%;'><h3 style='width: 100px; margin: auto;text-align: center; position: relative; padding-bottom: 15px;'>"+response.data[k].priority_type+"</h3>");
                                              planing=true;
                                        }
                                        }
...
function Accepted_view_page() {
   ...
   /*         $("#View_page_for_Accepted_task").append("<li data-icon='false'><a href='#'  class='wborderradius' data-transition='slide'>\
<div style='float:right;margin-top:10px;'><img src='img/checkcircle.png' class='' width='24' height='24'></span><p class='statusText'>&nbsp;" + response.data[0].status_type + "</p></div>\
<h2 class='viewHurdleDescription' style=''>" + response.data[0].task_name + "</h2>\
<p class='viewHurdleDescription2' style=''>" + weekday[parseInt(d.getDay())] + " , " + d.getDate() + " " + monthNames[parseInt(d.getMonth())] + "</p><hr>\
<p class='viewHurdleDescription1'><b>Address </b><br><p class='viewHurdleDescription2'> " + response.data[0].task_address + "</p></p>\
<p class='btnFontSizesecondary' style='color:" + localStorage.getItem("s_colorcode") + ";' onclick='Completed_View(" + response.data[0].id + ");'><b> TASK DETAIL </b></p>\
</a>\
</li>");*/
                                    ...
                                    $('#expand_accepted').on('click', function (e) {
                                        ...
                                        if(isExpanded){
                                         ...
                                         if(isExpanded)
                                              ...
                                              $('#expand_accepted').animate({bottom:size});
                                         $('html, body').animate({scrollTop:$(document).height()});
                                         return false;
...
function Accepted_End_trip_view_page() {
    ...
    QuinticaWebService("GET", "taskDetails", parameters, function(response) {
        if (response.code == "1") {
            ...
            var d = new Date(response.data[0].created_date);
           /* $("#View_page_End_trip_for_Accepted_task").append("<li data-icon='false'><a href='#' class='wborderradius' data-transition='slide'>\
...
<p class='viewHurdleDescription2'>" + weekday[parseInt(d.getDay())] + " , " + d.getDate() + " " + monthNames[parseInt(d.getMonth())] + "</p>\
<hr>\
<p class='viewHurdleDescription1'><b>Address</b><br><p class='viewHurdleDescription2'n> " + response.data[0].task_address + "</p></p>\
<hr>\
<p class='btnFontSizesecondary' style='color:" + localStorage.getItem("s_colorcode") + ";' onclick='Completed_View(" + response.data[0].id + ");'><b>TASK DETAIL</b></p>\
<p class='ui-li-aside' style='color:#E2D401'></p></a>\
</li>");*/
                                    ...
                                    $(function () {
                                                ...
                                                $('#expand_accepted_end').on('click', function (e) {
                                                    ...
                                                    if(isExpanded){
                                                     ...
                                                     $('#expand_accepted_end').animate({bottom:size});
                                                     $('html, body').animate({scrollTop:$(document).height()});
...
function Pending_reason_list() {
    navigator.notification.confirm("Are you sure you want to save this task on hold?", btnLabel_Pending_reason_list, "WorkWide", ["Yes", "No"]);
}
    ...
    QuinticaWebService("GET", "task_list", parameters, function(response) {
        ...
        if (response.code == "1") {
            ...
            for (k = 0; k < Object.keys(response.data).length; k++) {
                ...
                $("#pending_task_view_accepted").append("<li data-icon='false'><a href='#' onclick='Completed_View(" + response.data[k].id + ");' data-transition='slide' class='wborderradius'>\
<div style='float:right;margin-top:10px;'><img src='img/checkcircle.png' class='' width='24' height='24'><p class='statusText'>&nbsp;" + response.data[k].status_type + "</p></div>\
<h2 class='viewHurdleDescription' style=''> " + response.data[k].task_name + "</h2>\
...
function Accepted_to_rejection_task_list_reason_confirm_box() {
    ...
    if (main_comment_reject.trim() == "" || main_comment_reject.length <= 0) {
        ...
        navigator.notification.confirm("Are you sure you want to reject this task??", Accepted_to_Reject_task_list_reason_btn_lbl, "WorkWide", ["Yes", "No"]);
    }
}
...
function Update_assets() {
    ...
    if (inputValue == '') {
        ...
        //}
        // else{
        // showAlert("This Assets already Added, Please select another one", "WorkWide");
        // }
...
function sending_image_to_backend() {
	...
	if(checkConnection()) {
        ...
        if(isOfflineAppWorking){
            ...
            var folderName = cordova.file.applicationStorageDirectory;
...
function taskDetailspage() {
        ...
        if (response.code == "1") {
            ...
            $.each(obj, function(key, value) {
                ...
                if (key == "category" + i) {
                    ...
                    $("#complete_listview").append("<p class='taskdetails_fields'><b>" + key + "</b></p><p class='taskvalue_fields'>" + value + "</p>");
 ...
 function changeListner(textarea_reason, submit){
 if ($(textarea_reason).val() != "" || $(textarea_reason).val() != " ") {
...
//});
 ...
 function selectOnChangeListner(select_reason, submit){
  if ($(select_reason).val() != "0" || $(select_reason).val() != "-1" || $(select_reason).val() != -1) {
         $(submit).removeClass("ui-disabled");
...
function backNavigation() {
    ...
    if (pid == "category_view_page") {
        ...
        categorySumbit_offline_button_back();
    }else if(pid=="Accepted_Start_work_view_tasklist_page" || pid=="offline_direct_work_start"  ){
         return;
    }else if(pid=="attachmentPage"  ){
        changePage("#Accepted_Start_work_view_tasklist_page", "slide", false);
        return;
                ...
                if (apiFlag == -1) {
                    ...
                    for (i = 0; i < len; i++) {
                        ...
                        if (response.data[i].option_type == "TEXT") {
                            ...
                            //$("#" + catname).append("<label class='labelcolor' for='" + response.data[i].id + "'>" + response.data[i].label + required_status + "</label><input type='checkbox'  name='" + response.data[i].id + "' id='" + response.data[i].id + "' class='chekboxwidget " + requiredClass + "'>");
                            ...
                            console.log(response.data[i].option_type);
                            if (response.data[i].required_status == "1" || response.data[i].required_status == 1) {
...
function Accepted_End_task() {
    ...
    for (i = 0; i < catArray.length; i++) {
        ...
        $("#categoryContent").find("#" + catArray[i].replace(/ /g, "_") + "form").find(".requiredClass").each(function(k, v) {;
            if ($("." + z + k).val() == "" || $("." + z + k).val() == 0 || $("." + z + k).val() == "0" || $("." + z + k).val() == "null" || $("." + z + k).val() == null || $("." + z + k).val() == "off") {
    ...
    if (validationconfirmtask == false) {
        navigator.notification.confirm("Are you sure you want to end this task?", Accepted_End_task_button, "WorkWide", ["Yes", "No"]);
    } else {
...
try{
    ...
    }catch(error){console.log("ATTACHMENT MSG : "+error.message)}
    }, function(err) {
    console.log(err);
    });
    }, function(err) {
...
function category_details_offline() {
    ...
    QuinticaWebService("GET", "casorOffline", parameters, function(response) {
        if (response.data == null || response.data == "null") {
            ...
            var len = Object.keys(response.data).length;
            var db = window.openDatabase("database_Workwide", "1.0", "database_Workwide_table", 2 * 1024 * 1024)
            db.transaction(success_Workwide_category, error_Workwidee);
...
function Data_list_for_Assigned(tx, results) {
    ...
    for (var i = 0; i < len; i++) {
        ...
        var d = new Date(results.rows.item(i).updated_date);
        ...
        if(results.rows.item(i).priority=='1'){
                                                                    ...
                                                                    if(!heigh){
                                                                    $("#offline_Assigned_page").append("<hr data-icon='false' style='margin-top: 5%; background-color: #ff7e00; height: 2px; width: 50%;'><h3 style='padding-bottom: 20px; width: 100px; margin: auto;text-align: center; position: relative; padding-bottom: 15px;'>"+results.rows.item(i).priority_type+"</h3>");
                                                                    heigh=true;
                                                                    ...
                                                                    if(!moderate){
                                                                    $("#offline_Assigned_page").append("<hr data-icon='false' style='margin-top: 5%; background-color: #0c6410; height: 2px; width: 50%;'><h3 style='padding-bottom: 20px; width: 100px; margin: auto;text-align: center; position: relative; padding-bottom: 15px;'>"+results.rows.item(i).priority_type+"</h3>");
                                                                    moderate=true;
                                                                    }
                                                                    }
                                                                    if(results.rows.item(i).priority=='4'){
                                                                    if(!low){
                                                                    $("#offline_Assigned_page").append("<hr  data-icon='false' style='margin-top: 5%; background-color: #0836e2; height: 2px; width: 50%;'><h3 style='padding-bottom: 20px; width: 100px; margin: auto;text-align: center; position: relative; padding-bottom: 15px;'>"+results.rows.item(i).priority_type+"</h3>");
                                                                    low=true;
                                                                    }
                                                                    }
                                                                    ...
                                                                    if(!planing){
                                                                    $("#offline_Assigned_page").append("<hr data-icon='false' style='margin-top: 5%; background-color: #8808e2; height: 2px; width: 50%;'><h3 style='padding-bottom: 20px; width: 100px; margin: auto;text-align: center; position: relative; padding-bottom: 15px;'>"+results.rows.item(i).priority_type+"</h3>");
                                                                    planing=true;
                                                                    }
                                                                    }
...
<p class='viewHurdleDescription2'> " + weekday[d.getDay()] + " , " + d.getDate() + " " + monthNames[d.getMonth()] + "</p>\
<hr>\
...
function offline_Assigned_View_page_Success(tx, results) {
    ...
    var d = new Date(results.rows.item(0).updated_date);
...
<p class='viewHurdleDescription2' style=''>" + weekday[d.getDay()] + " , " + d.getDate() + " " + monthNames[d.getMonth()] + "</p><hr>\
...
function offline_assign_to_accepted() {
    navigator.notification.confirm("Are you sure you want to accept this task?", offline_assign_to_accepted_button, "WorkWide", ["Yes", "No"]);
}
...
<div style='float: right;margin-top:10px;'><img src='img/checkcircle.png' class='' width='24' height='24'><p class='statusText'>&nbsp;" + results.rows.item(i).status_type + "</p></div>\
<h2 class='viewHurdleDescription' style=''> " + results.rows.item(i).task_name + "</h2>\
<p class='viewHurdleDescription2'><span> " + weekday[d.getDay()] + " , " + d.getDate() + " " + monthNames[d.getMonth()] + "</span></p><hr>\
...
function data_list_for_Accepted(tx, results) {
    ...
    for (var i = 0; i < len; i++) {
        ...
        if(results.rows.item(i).priority=='1'){
                                                                    ...
                                                                    if(results.rows.item(i).priority=='2'){
                                                                    if(!heigh){
                                                                    $("#offline_Accepted_page").append("<hr data-icon='false' style='margin-top: 5%; background-color: #ff7e00; height: 2px; width: 50%;'><h3 style='padding-bottom: 20px; width: 100px; margin: auto;text-align: center; position: relative; padding-bottom: 15px;'>"+results.rows.item(i).priority_type+"</h3>");
                                                                    heigh=true;
                                                                    ...
                                                                    if(results.rows.item(i).priority=='3'){
                                                                    if(!moderate){
                                                                    $("#offline_Accepted_page").append("<hr data-icon='false' style='margin-top: 5%; background-color: #0c6410; height: 2px; width: 50%;'><h3 style='padding-bottom: 20px; width: 100px; margin: auto;text-align: center; position: relative; padding-bottom: 15px;'>"+results.rows.item(i).priority_type+"</h3>");
                                                                    moderate=true;
                                                                    }
                                                                    }
                                                                    if(results.rows.item(i).priority=='4'){
                                                                    if(!low){
                                                                    $("#offline_Accepted_page").append("<hr  data-icon='false' style='margin-top: 5%; background-color: #0836e2; height: 2px; width: 50%;'><h3 style='padding-bottom: 20px; width: 100px; margin: auto;text-align: center; position: relative;padding-bottom: 15px;'>"+results.rows.item(i).priority_type+"</h3>");
                                                                    low=true;
                                                                    }
                                                                    }
                                                                    ...
                                                                    if(results.rows.item(i).priority=='5'){
                                                                    if(!planing){
                                                                    $("#offline_Accepted_page").append("<hr data-icon='false' style='margin-top: 5%; background-color: #8808e2; height: 2px; width: 50%;'><h3 style='padding-bottom: 20px; width: 100px; margin: auto;text-align: center; position: relative; padding-bottom: 15px;'>"+results.rows.item(i).priority_type+"</h3>");
                                                                    planing=true;
                                                                    }
                                                                    }
...
<div style='float:right;margin-top:10px;'><img src='img/checkcircle.png' class='' width='24' height='24'><p class='statusText'>&nbsp;" + results.rows.item(i).status_type + "</p></div>\
<h2 class='viewHurdleDescription' style=''> " + results.rows.item(i).task_name + "</h2>\
<p class='viewHurdleDescription2' style=''> " + weekday[d.getDay()] + " , " + d.getDate() + " " + monthNames[d.getMonth()] + "</p>\
<hr>\
<p class='viewHurdleDescription1'><b>Address</b></p><p class='viewHurdleDescription2'> " + results.rows.item(i).task_address + "</p>\
</li>");
    }
...
function _category_offline_mode() {
    var db = window.openDatabase("database_Workwide", "1.0", "database_Workwide_table", 2 * 1024 * 1024)
...
function signature_page_navigation() {
    ...
    if (validationconfirmtask == false) {
        navigator.notification.confirm("Are you sure you want to end this task?", signature_page_navigation_success, "WorkWide", ["Yes", "No"]);
    } else {
        catArray1 = $.unique(catArray1);
        for (i = 0; i < catArray1.length; i++) {
            if (i == catArray1.length - 1) {
...
function btnLabel_signature_page_navigation(buttonIndex) {
...
function Reject_reason_list_offline() {
    navigator.notification.confirm("Are you sure you want to reject this task?", btnLabel_Reject_reason_list_offline, "WorkWide", ["Yes", "No"]);
}
...
function offline_status_change_in_inprogress_to_on_reject() {
    navigator.notification.confirm("Are you sure you want to reject this task?", offline_status_change_in_inprogress_to_on_reject_btn_lbl_success, "WorkWide", ["Yes", "No"]);
}
        ...
        if (temp == "category_data") {
            ...
            $.each(obj1, function(k, v) {
                ...
                if (k == "category" + i) {
                    ...
                    $("#offline_complete_listview").append("<p class='taskdetails_fields'><b>" + k + "</b></p><p class='taskvalue_fields'>" + v + "</p>");
                }
            ...
            $("#offline_complete_listview").append("<p class='taskdetails_fields'><b>" + key + "</b></p><p class='taskvalue_fields'>" + value + "</p>");
        }
        }
       }catch(error){}
    });
    ...
    if (buttonIndex == 1) {
        ...
        _sending_data_to_backend();
               setTimeout(function() {
                   ...
                   loopval=0;
                   UploadAttachmentstoserver(cordova.file.applicationStorageDirectory);
...
function getSuggestions() {
    var text = $("#myInputsss_offline").val();
...
function _retrieve_Assets_taking_value_from_Array_suucess_one(tx, results) {
    ...
    if (rassets == null) {
        ...
        for (i = 0; i < Object.keys(rassets).length; i++) {
            if (rassets[i].ID != "undefined" || rassets[i].description != "undefined" || rassets[i].used != "undefined" || rassets[i].awaiting != "undefined")
                $("#Assets_ul_server_offline").append("<li data-icon='false' class='textColor' style='border-radius:0px !important;border-bottom: 1px solid lightgray;'><a href='#'><h4>" + rassets[i].ID + "</h4><h6>" + rassets[i].description + "</h6><h6>Used: " + rassets[i].used + "</h6><h6>Awaiting: " + rassets[i].awaiting + "</h6></a></li>");
        }
    }
